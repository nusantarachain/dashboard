...
import { ADDRESS_PREFIX, ORG_PREFIX, SEED_PREFIX } from './constants';
import { QrScan } from './Scan';

const NUCHAIN_PREFIX = 'nuchain';
    ...
    (data: string | null): void => {
      if (data) {
        try {
          ...
          const isValidPrefix = prefix === ADDRESS_PREFIX || prefix === NUCHAIN_PREFIX || prefix === SEED_PREFIX || ORG_PREFIX;

          assert(isValidPrefix, `Invalid prefix received, expected '${ADDRESS_PREFIX}/${SEED_PREFIX}' , found '${prefix}'`);

          const isAddress = prefix === ADDRESS_PREFIX || prefix === NUCHAIN_PREFIX || prefix == ORG_PREFIX;
